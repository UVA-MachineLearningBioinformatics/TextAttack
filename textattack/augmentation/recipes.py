...
class CoverageGuidedAugmenter(Augmenter):
		...
		def __init__(self, test_model, K=2, iterations = 10, transformations_per_example = 1, \
				outfile = 'aug.csv', savedir = './', beam_size = -1, epsilon=0.0001,key = 'text', language_model = 'gpt2',
				 max_seq_len = -1,	pct_words_to_swap=0.1, threshold = 0.5, select_swap_parameter = False, beam = 1, perplexity_tolerance = 0.5):
				assert (
				...
				self.nc_test_model = neuronCoverage(self.test_model, threshold=threshold, max_seq_len=max_seq_len)
				self.perplexity_calculator = PerplexityCoverage(language_model=language_model, tokenizer=self.test_model, max_seq_len=max_seq_len)
				self.K = K
		...
		def augment_many(self, seed_texts, labels):
				...
				while len(text_queue)>0:
						...
						baseline_yhat = torch.max(self.nc_test_model._eval(current_seed_text)[0], dim=-1)[1]
						# ground truth of sample
						#truth = current_seed_text[1]
						
						num_generated = 0 # to limit the number of test samples generated per sample
						if len(text_stack) == 0:
						...
						while len(text_stack)>0:
								...
								for i in range(self.iterations):
												...
												sampled_transformations = random.sample(list(self.available_transformations), self.K)
												...
												for transformation in sampled_transformations:
													if isinstance(transformation,textattack.transformations.WordSwapEmbedding):
														constraints = DEFAULT_CONSTRAINTS + [textattack.constraints.semantics.WordEmbeddingDistance(min_cos_sim=0.8)]
													else:
														constraints = DEFAULT_CONSTRAINTS
													
													tid.append(Augmenter(transformation=transformation(), constraints=constraints, 
													pct_words_to_swap=pct_words_to_swap, transformations_per_example=self.transformations_per_example))
												...
												print('original perplexity:', current_seed_perp, "new perplexity:", self.perplexity_calculator([new_text]))
												
												if abs(self.perplexity_calculator([new_text])) > abs(current_seed_perp):
													augment_perplexity = abs(abs(self.perplexity_calculator([new_text])) / abs(current_seed_perp))
												else:
													augment_perplexity = abs( abs(current_seed_perp)/ abs(self.perplexity_calculator([new_text])) )
												# prediction for augmented text
												print("ratio of perpelxity: ", augment_perplexity)
												augment_yhat = torch.max(self.nc_test_model._eval(new_text)[0], dim=-1)[1]
												print("new neuron coverage:", new_neuron_coverage," original:",  current_neuron_coverage)
												print("does this increase neuron coverage?", (new_neuron_coverage - current_neuron_coverage) > self.epsilon)
												print("is this a good sentence?", augment_perplexity<self.perplexity_tolerance)
												if (new_neuron_coverage - current_neuron_coverage) > self.epsilon and (augment_perplexity<self.perplexity_tolerance):
													...
													if current_neuron_coverage > 0.9999:
														return generated_test, generated_test_label, [original_neuron_coverage, current_neuron_coverage]
